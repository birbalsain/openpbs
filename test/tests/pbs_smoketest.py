...
class SmokeTest(PBSTestSuite):
    ...
    def test_fairshare_enhanced(self):
        ...
        fs1 = self.firshare.query_fairshare(str(TEST_USER1), None, self.scheduler.has_snap,
                                                     self.scheduler.pbs_conf,
                                                     self.scheduler.sc_name, self.scheduler.hostname,
                                                     self.scheduler.fs_tag)
        self.logger.info('Checking ' + str(fs1.usage) + " == 3")
        self.assertEqual(fs1.usage, 3)
        fs2 = self.scheduler.query_fairshare(str(TEST_USER2), None, self.scheduler.has_snap,
                                                     self.scheduler.pbs_conf,
                                                     self.scheduler.sc_name, self.scheduler.hostname,
                                                     self.scheduler.fs_tag)
        self.logger.info('Checking ' + str(fs2.usage) + " == 3")
        self.assertEqual(fs2.usage, 3)
        fs3 = self.scheduler.query_fairshare(str(TEST_USER3), None, self.scheduler.has_snap,
                                                     self.scheduler.pbs_conf,
                                                     self.scheduler.sc_name, self.scheduler.hostname,
                                                     self.scheduler.fs_tag)
        self.logger.info('Checking ' + str(fs3.usage) + " == 3")
        self.assertEqual(fs3.usage, 3)
        fs4 = self.scheduler.query_fairshare(str(TEST_USER4), None, self.scheduler.has_snap,
                                                     self.scheduler.pbs_conf,
                                                     self.scheduler.sc_name, self.scheduler.hostname,
                                                     self.scheduler.fs_tag)
        ...
        # query fairshare and check usage
        fs1 = self.scheduler.query_fairshare(str(TEST_USER1), None, self.scheduler.has_snap,
                                                     self.scheduler.pbs_conf,
                                                     self.scheduler.sc_name, self.scheduler.hostname,
                                                     self.scheduler.fs_tag)
