...
/**
 * @brief
 * 	information of job's output/error file
 *
 * @param[in] preq - pointer to batc_request structure
 *
 * @return 	Void
...
req_reqinfo(struct batch_request *preq)
{
	...
	gid_t	exgid;
	char	*file_name;
	pjob = find_job(preq->rq_ind.rq_reqinfo.rq_jid);

	if (pjob == NULL){
		ret = PBSE_UNKJOBID;
		goto last;}

	if ((preq->rq_ind.rq_reqinfo.rq_info == PBS_BATCH_FileOpt_Default) ||
		(preq->rq_ind.rq_reqinfo.rq_info & PBS_BATCH_reqinfo_OFlg)) {
		jft = StdOut;
	}
	else if (preq->rq_ind.rq_reqinfo.rq_info & PBS_BATCH_reqinfo_EFlg) {
		jft = StdErr;
	}
	/* must be Mother Superior for this to make sence */
	if ((pjob->ji_qs.ji_svrflags & JOB_SVFLG_HERE) == 0){
	...
	if (pjob->ji_grpcache == NULL) {
		sprintf(log_buffer, "error in grapche");
		log_event(PBSEVENT_JOB, PBS_EVENTCLASS_JOB, LOG_DEBUG,
		pjob->ji_qs.ji_jobid, log_buffer);
		ret = -1;
		goto last;}	/* needs to be non-NULL for figuring out homedir path; otherwise, mom will crash! */

	file_name = std_file_name(pjob, jft, &keeping);
	pid_t pid;
	pid = fork();
	if(pid == 0) {
		int status;
		...
		if (sockfd == -1) 
			exit(PBSE_SOCKETFAIL);

		bzero(&servaddr, sizeof(servaddr));

		sprintf(log_buffer, "qview requested file %s",file_name);
		log_event(PBSEVENT_JOB, PBS_EVENTCLASS_JOB, LOG_DEBUG,
			pjob->ji_qs.ji_jobid, log_buffer);
		// assign IP, PORT
		servaddr.sin_family = AF_INET;
		servaddr.sin_addr.s_addr = inet_addr(preq->rq_ind.rq_reqinfo.rq_IPadd); 
		servaddr.sin_port = htons(preq->rq_ind.rq_reqinfo.rq_Port);
		// connect the client socket to server socket 
		if (connect(sockfd, (SA*)&servaddr, sizeof(servaddr)) != 0)
			exit(PBSE_CONNECTION_WITH_SERVER_FAIL);

		char start_msg[] = "MOM is sending job information\n";
		char end_msg[] = "Job finished\n";
		send(sockfd, start_msg, sizeof(start_msg), 0);
		char *cat_args[] = {"tail", "-f", file_name, NULL};
		/* duplicate socket on stdout */
		dup2( sockfd, STDOUT_FILENO );
		/* tail file info to requested client socket */
		execvp(*cat_args, cat_args);
		exit(0);
	}
	if (pjob->ji_qview_pid_count == pjob->ji_qview_array_size) {
		pjob->ji_qview_array_size = 2*pjob->ji_qview_array_size;
		pjob->arr = (pid_t *)realloc(pjob->arr,pjob->ji_qview_array_size*sizeof(pid_t));
	}

	*(pjob->arr + pjob->ji_qview_pid_count) = pid;
	sprintf(log_buffer, "pid is %d", (int)(*(pjob->arr + pjob->ji_qview_pid_count)));
	log_event(PBSEVENT_JOB, PBS_EVENTCLASS_JOB, LOG_DEBUG,
		pjob->ji_qs.ji_jobid, log_buffer);
	pjob->ji_qview_pid_count++;
	last:
	if (ret == PBSE_NONE)
		reply_ack(preq);
	else
